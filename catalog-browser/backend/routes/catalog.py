"""Rotas da API: catálogos, schemas, tabelas (filtro motiva)."""
...
@router.get("/catalogs")
def list_catalogs():
    """Lista catálogos com prefixo configurado (ex: motiva)."""
    w = get_client()
    catalogs = []
...
def list_schemas(catalog_name: str):
    """Lista schemas de um catálogo (apenas se o catálogo for motiva_*)."""
...
def list_tables(catalog_name: str, schema_name: str):
    """Lista tabelas de um schema (apenas catálogos motiva_*)."""
    if not _catalog_matches(catalog_name):
        raise HTTPException(status_code=404, detail="Catalog not allowed")
    w = get_client()
    tables = []
